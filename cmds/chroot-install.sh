...
arch-chroot /mnt/hexblade/installer apt -y install locales keyboard-configuration 
...
arch-chroot /mnt/hexblade/installer apt -y install kali-linux-core \
  software-properties-common \
  vim wget curl openssl git vim \
  nmap ncat pv zip connect-proxy tcpdump bc \
  network-manager net-tools # netcat debconf-utils
...
DEBIAN_FRONTEND=noninteractive arch-chroot /mnt/hexblade/installer apt -y install --install-recommends linux-image-amd64
